...
**Nikan.Developer** هي لوحة تحكم ويب متقدمة ومفتوحة المصدر لإدارة خوادم [Xray-core](https://github.com/XTLS/Xray-core). توفّر واجهة نظيفة ومتعددة اللغات لنشر وتكوين ومراقبة مجموعة واسعة من بروتوكولات الوكيل وVPN — من خادم VPS واحد إلى عمليات النشر متعددة العقد.

تم بناء Nikan.Developer كنسخة محسّنة (fork) من مشروع Nikan.Developer الأصلي، وتضيف دعمًا أوسع للبروتوكولات، واستقرارًا محسّنًا، ومحاسبة للترافيك لكل عميل، والعديد من ميزات تحسين تجربة الاستخدام.
...
أثناء التثبيت، يتم إنشاء اسم مستخدم وكلمة مرور ومسار وصول عشوائية. بعد التثبيت، شغّل `nikan-developer` لفتح قائمة الإدارة، حيث يمكنك بدء/إيقاف الخدمة، وعرض أو إعادة تعيين بيانات تسجيل الدخول، وإدارة شهادات SSL، والمزيد.

للحصول على الوثائق الكاملة، يرجى زيارة [ويكي المشروع](https://github.com/NikanDeveloper56/Nikan.Developer/wiki).

### التثبيت غير التفاعلي

يعمل المثبِّت أيضًا **بشكل غير تفاعلي** لـ cloud-init.
عيّن `NikanDeveloper_NONINTERACTIVE=1` (أو مرّره عبر أنبوب دون TTY) وسيتولى التثبيت من البداية إلى النهاية
دون أي مطالبات، مُنشئًا بيانات اعتماد عشوائية وكاتبًا إياها في
`/etc/nikan-developer/install-result.env`. راجع [`deploy/`](deploy/) لـ:
...
يدعم Nikan.Developer خلفيتين (backends) يتم اختيارهما أثناء التثبيت:

- **SQLite** (افتراضي) — ملف واحد في `/etc/nikan-developer/nikan-developer.db`. بدون إعداد، مثالي لعمليات النشر الصغيرة والمتوسطة.
...
في وقت التشغيل، يتم اختيار الخلفية عبر متغيرات البيئة (يكتبها المثبِّت لك في `/etc/default/nikan-developer`):
...
# ثم عيّن NikanDeveloper_DB_TYPE و NikanDeveloper_DB_DSN في /etc/default/nikan-developer وأعد التشغيل:
...
يستمر الأمر الافتراضي `docker compose up -d` في استخدام SQLite. للتشغيل مع خدمة PostgreSQL المرفقة، أزِل التعليق عن سطري متغيرات البيئة `NikanDeveloper_DB_*` في `docker-compose.yml` وشغّل باستخدام البروفايل:
...
| --- | --- | --- |
| `NikanDeveloper_DB_TYPE` | خلفية قاعدة البيانات: `sqlite` أو `postgres` | `sqlite` |
| `NikanDeveloper_DB_DSN` | سلسلة اتصال PostgreSQL (عندما `NikanDeveloper_DB_TYPE=postgres`) | — |
| `NikanDeveloper_DB_FOLDER` | مجلد ملف قاعدة بيانات SQLite | `/etc/nikan-developer` |
| `NikanDeveloper_DB_MAX_OPEN_CONNS` | الحد الأقصى للاتصالات المفتوحة (تجمّع PostgreSQL) | — |
| `NikanDeveloper_DB_MAX_IDLE_CONNS` | الحد الأقصى للاتصالات الخاملة (تجمّع PostgreSQL) | — |
| `NikanDeveloper_INIT_WEB_BASE_PATH` | مسار URI الأولي للوحة الويب | `/` |
| `NikanDeveloper_ENABLE_FAIL2BAN` | تفعيل فرض حدود IP المعتمد على Fail2ban | `true` |
| `NikanDeveloper_LOG_LEVEL` | مستوى السجل (`debug`، `info`، `warning`، `error`) | `info` |
| `NikanDeveloper_DEBUG` | تفعيل وضع التصحيح | `false` |
| `NikanDeveloper_TUNNEL_HEALTH_MONITOR` | تفعيل مراقب صحة النفق (يفحص عنوان URL ويعيد تشغيل xray بعد فشل متكرر؛ إعادة التشغيل تقطع جميع العملاء) | `false` |
| `NikanDeveloper_TUNNEL_HEALTH_PROXY` | الوكيل الذي يُرسَل عبره الفحص؛ وجّهه إلى اتصال xray وارد محلي ليختبر الفحص النفق (مثل `socks5://127.0.0.1:1080`). القيمة الفارغة تعني أن الفحص يتحقق فقط من اتصال المضيف | — |
| `NikanDeveloper_TUNNEL_HEALTH_URL` | عنوان URL الذي يُفحَص لمعرفة صحة النفق | `https://www.cloudflare.com/cdn-cgi/trace` |
| `NikanDeveloper_TUNNEL_HEALTH_INTERVAL` | الفترة بين عمليات الفحص | `30s` |
| `NikanDeveloper_TUNNEL_HEALTH_TIMEOUT` | مهلة كل عملية فحص | `10s` |
| `NikanDeveloper_TUNNEL_HEALTH_FAILURES` | عدد حالات الفشل المتتالية قبل تشغيل إعادة التشغيل | `3` |
| `NikanDeveloper_TUNNEL_HEALTH_COOLDOWN` | الحد الأدنى للتأخير بين عمليات إعادة التشغيل المتتالية | `5m` |
...
أدوات وتكاملات بناها المجتمع حول Nikan.Developer.

- [terraform-provider-Nikan.Developer](https://github.com/batonogov/terraform-provider-threexui) (الترخيص: **MIT**): _إدارة الاتصالات الواردة والعملاء وإعدادات اللوحة وتكوين Xray كرمز باستخدام Terraform / OpenTofu._
