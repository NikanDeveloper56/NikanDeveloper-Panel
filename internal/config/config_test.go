...
func TestGetPortOverride(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if tt.set {
				t.Setenv("NikanDeveloper_PORT", tt.value)
			} else {
				original, existed := os.LookupEnv("NikanDeveloper_PORT")
				if err := os.Unsetenv("NikanDeveloper_PORT"); err != nil {
					t.Fatalf("unset NikanDeveloper_PORT: %v", err)
				}
				t.Cleanup(func() {
					if existed {
						_ = os.Setenv("NikanDeveloper_PORT", original)
					} else {
						_ = os.Unsetenv("NikanDeveloper_PORT")
					}
				})
			}
