# Nikan.Developer on Hetzner Cloud

Hetzner Cloud does **not** have a third-party image marketplace the way AWS does.
Ship Nikan.Developer via **cloud-init**: each instance installs non-interactively and
...
Nikan.Developer non-interactively and generates unique per-instance credentials.
...
```bash
ssh root@<server-ip> 'cat /etc/nikan-developer/install-result.env'
```
...
is essentially a documented cloud-init config plus metadata. To propose Nikan.Developer as
